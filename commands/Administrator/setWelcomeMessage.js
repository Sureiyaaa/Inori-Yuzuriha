...
exports.run = async (client, message, args) => {  
  ...
  const [...value] = args;
      let botembed = new Discord.RichEmbed()
        ...
        .setTitle("Set Config")
        .setDescription(`Guild configuration **Welcome Message** has been set to:\n\`${value.join(" ")}\``)
        .setColor(color.inori)
        .setTimestamp()
  client.settings.set(message.guild.id, value.join(" "), "welcomeMessage");
  message.channel.send(botembed);
}
exports.conf = {
  enabled: true,
  guildOnly: false,
  aliases: ["swm", "setMessage"],
};
