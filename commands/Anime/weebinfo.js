...
async function profile(message, score) {
  ...
  let status = await db.get(`userStatus_${user.id}`);
  if(status === null) status = "None";
    let Waifu = await db.get(`userWaifu_${user.id}`);
  if (Waifu === null) Waifu == "None";
  
  let targetBalance = await db.get(`userBalance_${user.id}`);
  if (targetBalance === null) targetBalance == 0;
  let reset = await db.get(`userReset_${user.id}`);
