...
const Youtube_API_Key = settings.ytapikey
...
=======================================\nChoose Song Number from 1-5.\`\`\``)
  ...
  const dispatcher = queue.get(guild.id).connection.playStream(ytdl(song.url, {filter:'audioonly', quality: 'highestaudio'}, {passes: 3, bitrate: 192}))
		.on('end', reason => {
